...
public class BankAccount {
    ...
    private boolean canWithdraw(double withdrawAmount) {
        boolean isPositiveAmount = withdrawAmount >= 0;
        boolean hasSufficientFunds = balance >= withdrawAmount;
        boolean isBelowSingleLimit = withdrawAmount < withdrawLimit;
        boolean isBelowTotalLimit = (withdrawAmount + amountWithdrawn) <= withdrawLimit;

        // CC value = 3 + 1 = 4
        return isPositiveAmount && hasSufficientFunds && isBelowSingleLimit && isBelowTotalLimit;
    }
